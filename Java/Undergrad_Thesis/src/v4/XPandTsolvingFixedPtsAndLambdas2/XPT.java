package XPandTsolvingFixedPtsAndLambdas2;
...
public abstract class XPT extends AbstractCalculation
{
	...
	public static void main(String[] args) throws IOException
	{
		...
		double kx = variable[3] = 0.1;
		double rp0 = variable[4] = 1;
		double rp1 = variable[5] = 1;
		double kp = variable[7] = 0.01;
		double kt1 = variable[8] = 1; //rt1
		double kt2 = variable[9] = 0.01; //kt
		
		//title of text file
	    String result = "rx0_" + variable[0] + "-" + 10*variable[0] + " rx2_" + variable[1] + "-" + 10*variable[1] + " p0_" + variable[2] 
	    		+ " kx_" + variable[3] + "-" + 10*variable[3] + " rp0_" + variable[4] + "-" + 10*variable[4] + " rp1_" + variable[5] + "-" + 10*variable[5] + " A_" + variable[6] 
	    		+ " kp_" + variable[7] + "-" + 10*variable[7] + " rt1_" + variable[8] + "-" + 10*variable[8] + " kt_" + variable[9] + "-" + 10*variable[9];
		
	    //creates file **remember to change the folder every time you run!!!
		FileWriter fw1 = null;
		File newTextFile1 = new File("C:\\Thesis Results\\run set 6\\" + result + ".txt");
		...
		for(rx0 = variable[0]; rx0 <= 10*variable[0]; rx0 += 0.1)
		{
			for(rx2 = variable[1]; rx2 <= 10*variable[1]; rx2 += 0.01)
			{
				//for(p0 = variable[2]; p0 <= 10*variable[2]; p0 += 5.0)
				//{
				
					for(kx = variable[3]; kx <= 10*variable[3]; kx += 0.1)
					{
						for(rp0 = variable[4]; rp0 <= 10*variable[4]; rp0 += 1.0)
						{
							for(rp1 = variable[5]; rp1 <= 10*variable[5]; rp1 += 1.0)
							{
								//for(A = variable[6]; A <= 10*variable[6]; A += 1.0)
								//{
									for(kp = variable[7]; kp <= 10*variable[7]; kp += 0.01)
									{
										for(kt1 = variable[8]; kt1 <= 10*variable[8]; kt1 += 1.0)
										{	
											for(kt2 = variable[9]; kt2 <= 10*variable[9]; kt2 += 0.01)
											{
												...
												if (v>0 && w>0 && u>0)
												{
													...
													if (delta < 0)
													{
														...
														if (sqroot > 0)
														{
															...
															if(variable2[14] >= 0 && ((variable2[15]>150)&&(variable2[15]<250)))
															{
																writeToFile(variable2, fw1);
																//writeToFile(variable3, fw1);
															}
															if(variable3[14] >= 0 && ((variable3[15]>150)&&(variable3[15]<250)))
															{
																writeToFile(variable3, fw1);
																//writeToFile(variable2, fw1);
															}
	...
	public static void writeToFile(double[] var, /*double[] grid2,*/ FileWriter fw1) throws IOException
	{
		//print off order:
		//rx0   rx2   p0   kx   rp0   rp1   A   kp   rt1   kt   p*   x*   t*   Lr   Li   Period
